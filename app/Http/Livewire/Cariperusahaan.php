...
class Cariperusahaan extends Component
{
    ...
    public function docariperusahaan()
    {
        ...
        $this->daftar = Perusahaan::select('perusahaans.id', 'perusahaans.created_at', 'perusahaans.badan_hukum',  'perusahaans.nama as perusahaan_nama', 'propinsis.nama as propinsi_nama', DB::raw('count(DISTINCT (plants.id)) as jumlah_plant'), DB::raw('count(perusahaanproduks.id) as jumlah_produk'))
                    ...
                    ->groupBy('perusahaans.id', 'perusahaans.badan_hukum',  'perusahaans.nama', 'propinsis.nama', 'perusahaans.created_at')
                    ->orderBy('perusahaans.nama', 'asc')
                    ->get();
        foreach($this->daftar as $key_perusahaan=>$perusahaan)
        {
    ...
    {
        $epoch_created_begin = ($this->epoch_created_begin === null ? '1971/1/1' : $this->epoch_created_begin);
        $epoch_created_end = ($this->epoch_created_end === null ? date('Y/m/d',time()) : $this->epoch_created_end);
        $whereAlamatPropinsi = ($this->propinsi_id === null ? "perusahaans.alamat_propinsi is not null" : "perusahaans.alamat_propinsi = $this->propinsi_id");
        $whereProdukId = ($this->produk_id === null ? "perusahaanproduks.produk_id is not null" : "perusahaanproduks.produk_id = $this->produk_id");
        $rekapitulasi = [];
        $this->daftar = Perusahaan::select('perusahaans.id', 'perusahaans.created_at', 'perusahaans.badan_hukum',  'perusahaans.nama as perusahaan_nama', 'propinsis.nama as propinsi_nama', DB::raw('count(DISTINCT (plants.id)) as jumlah_plant'), DB::raw('count(perusahaanproduks.id) as jumlah_produk'))
                    ->leftJoin('propinsis', 'perusahaans.alamat_propinsi', 'propinsis.id')
                    ->leftJoin('plants', 'perusahaans.id', 'plants.perusahaan_id')
                    ->leftJoin('perusahaanproduks', 'plants.id', 'perusahaanproduks.plant_id')
                    ->whereRaw("perusahaans.created_at::date between '$epoch_created_begin' and '$epoch_created_end'")
                    ->whereRaw($whereAlamatPropinsi)
                    ->whereRaw($whereProdukId)
                    ->groupBy('perusahaans.id', 'perusahaans.badan_hukum',  'perusahaans.nama', 'propinsis.nama', 'perusahaans.created_at')
                    ->orderBy('perusahaans.nama', 'asc')
                    ->get();
                    // ->toSql();
